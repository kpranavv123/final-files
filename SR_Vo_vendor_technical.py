...
FIELD_REASON = {
    ...
    "DESTINATIONPLANT":               "",   # sub-rows carry reasons
...
class POFromVendRuleEngine:
    ...
    def validate_purchaseorder(self, row) -> str:
        if self._is_blank(row.get("PURCHASEORDER")):
            return "PURCHASEORDER: Field is blank"
        return ""
    ...
    def validate_purchasingdocumenttype(self, row) -> str:
        if self._is_blank(row.get("PURCHASINGDOCUMENTTYPE")):
            return "PURCHASINGDOCUMENTTYPE: Field is blank"
        return ""

    # ── PURCHASEORDERITEM ─────────────────────
    def validate_purchaseorderitem(self, row) -> str:
        if self._is_blank(row.get("PURCHASEORDERITEM")):
            return "PURCHASEORDERITEM: Field is blank"
        return ""
    ...
    def validate_sourceplant(self, row) -> str:
        if self._is_blank(row.get("SOURCEPLANT")):
    ...
    def validate_schedulelinedeliverydate(self, row) -> str:
        ...
        if not self._valid_date(val):
            return (
                f"SCHEDULELINEDELIVERYDATE: '{str(val).strip()}' does not follow "
    ...
    def validate_netpriceinpurchasingdocumenti(self, row) -> str:
        if self._is_blank(row.get("NETPRICEINPURCHASINGDOCUMENTI")):
            return "NETPRICEINPURCHASINGDOCUMENTI: Field is blank"
        return ""

    # ── MATSTAGINGAVAILABILITYDATE ────────────
    def validate_matstagingavailabilitydate(self, row) -> str:
        val = row.get("MATSTAGINGAVAILABILITYDATE", "")
        if self._is_blank(val):
            return "MATSTAGINGAVAILABILITYDATE: Field is blank"
        if not self._valid_date(val):
            return (
                f"MATSTAGINGAVAILABILITYDATE: '{str(val).strip()}' does not follow "
                "the required format YYYYMMDD"
            )
        return ""
...
#  Validator
# ══════════════════════════════════════════════
class POFromVendTableValidator:

    def __init__(self, sr_path: str, partfg_path: str, site_path: str):
    ...
    def get_destinationplant_error_subcounts(self) -> dict:
        ...
        for idx, col_reason in self.reason_map.items():
            reason = col_reason.get("DESTINATIONPLANT", "")
            if not reason:
                continue
    ...
    def get_schedulelinedeliverydate_error_subcounts(self) -> dict:
        ...
        for idx, col_reason in self.reason_map.items():
            reason = col_reason.get("SCHEDULELINEDELIVERYDATE", "")
            if not reason:
                continue
            if "blank" in reason.lower():
                counts["blank"] += 1
    ...
    def get_matstagingavailabilitydate_error_subcounts(self) -> dict:
        counts = {"blank": 0, "bad_format": 0}
        for idx, col_reason in self.reason_map.items():
            reason = col_reason.get("MATSTAGINGAVAILABILITYDATE", "")
            if not reason:
                continue
            if "blank" in reason.lower():
                counts["blank"] += 1
...
class POFromVendReportWriter:
    ...
    def _write_summary_sheet_into(self, ws, error_map: dict, total_rows: int):
        ...
        title_cell           = ws.cell(row=1, column=1,
                                       value="ScheduledReceipt (PO from Vendor) Validation Summary")
        ...
        for col_name in FIELD_ORDER:
            ...
            # ── DESTINATIONPLANT sub-rows ──
            if col_name == "DESTINATIONPLANT" and has_errors:
                row_num = self._write_sub_rows(ws, row_num, [
                    ("  ↳ Blank Destination Plant",  dest_subcounts["blank"],
                     "DESTINATIONPLANT: Field is blank"),
                    ("  ↳ Not in Site Master",        dest_subcounts["not_in_site"],
            ...
            if col_name == "SCHEDULELINEDELIVERYDATE" and has_errors:
                row_num = self._write_sub_rows(ws, row_num, [
                    ("  ↳ Blank Schedule Line Delivery Date", sld_subcounts["blank"],
                     "SCHEDULELINEDELIVERYDATE: Field is blank"),
                    ("  ↳ Invalid Format (not YYYYMMDD)",     sld_subcounts["bad_format"],
    ...
    def _write_rules_sheet(self, wb):
        ...
        title_cell           = ws.cell(row=1, column=1,
                                       value="ScheduledReceipt (PO from Vendor) – Validation Rules")
...
#  Orchestrator
# ══════════════════════════════════════════════
class POFromVendTableProcessor:

    def __init__(self, sr_path: str, partfg_path: str, site_path: str, output_path: str):
        self.validator = POFromVendTableValidator(sr_path, partfg_path, site_path)
        self.writer    = POFromVendReportWriter(self.validator, output_path)

    def run(self):
        print("📂  Loading files …")
        self.validator.load()
        print(f"    PO_from_Vend columns detected : {list(self.validator.df.columns)}")
...
if __name__ == "__main__":
    processor = POFromVendTableProcessor(
        sr_path     = SR_INPUT_FILE,
