...
#  FILE PATHS
# ─────────────────────────────────────────────
ALLOCATION_INPUT_FILE = r"C:\Users\SW526XH\Downloads\Go Live-2\Allocation\Allocation.tab"
PARTFG_INPUT_FILE     = r"C:\Users\SW526XH\Downloads\Go Live-2\Allocation\Part_FG.xlsx"
PARTRM_INPUT_FILE     = r"C:\Users\SW526XH\Downloads\Go Live-2\Allocation\Part_RM.xlsx"
SITE_INPUT_FILE       = r"C:\Users\SW526XH\Downloads\Go Live-2\Allocation\Site_2026-04-09-1058.xlsx"
OUTPUT_FILE           = r"C:\Users\SW526XH\Downloads\Go Live-2\Allocation\Validated_Allocation.xlsx"
...
# ─────────────────────────────────────────────
KEEP_COLS = [
    "ORDERNUMBER", "PLANT", "BOMITEMNUMBER", "MATERIALNUMBER",
    "REQUIREMENTSDATEFORCOMPONENT", "REMAININGQTY", "REQUIREMENTQUANTITYINBUOM",
]
...
FIELDS_WITH_SUB_ROWS = {"PLANT", "MATERIALNUMBER", "REQUIREMENTSDATEFORCOMPONENT"}

# Per-field single-line reason shown in summary (blank for sub-row fields)
FIELD_REASON = {
    "ORDERNUMBER":                   "ORDERNUMBER: Field is blank",
    "PLANT":                         "",   # sub-rows carry reasons
    "BOMITEMNUMBER":                 "BOMITEMNUMBER: Field is blank",
    "MATERIALNUMBER":                "",   # sub-rows carry reasons
    "REQUIREMENTSDATEFORCOMPONENT":  "",   # sub-rows carry reasons
    "REMAININGQTY":                  "REMAININGQTY: Field is blank",
    "REQUIREMENTQUANTITYINBUOM":     "REQUIREMENTQUANTITYINBUOM: Field is blank",
}
...
class AllocationRuleEngine:
    ...
    def __init__(self, part_fg_materials: set, part_rm_materials: set, site_plants: set):
        self.part_fg_materials = set(str(m).strip().upper() for m in part_fg_materials)
        self.part_rm_materials = set(str(m).strip().upper() for m in part_rm_materials)
    ...
    # ── ORDERNUMBER ───────────────────────────
    def validate_ordernumber(self, row) -> str:
        if self._is_blank(row.get("ORDERNUMBER")):
            return "ORDERNUMBER: Field is blank"
        return ""
    ...
    # ── BOMITEMNUMBER ─────────────────────────
    def validate_bomitemnumber(self, row) -> str:
        if self._is_blank(row.get("BOMITEMNUMBER")):
            return "BOMITEMNUMBER: Field is blank"
        return ""
    ...
    def validate_materialnumber(self, row) -> str:
        ...
        if val_str not in self.part_fg_materials and val_str not in self.part_rm_materials:
            return (
                f"MATERIALNUMBER: '{val_str}' is not present in Part(FG) or Part(RM) master"
            )
        return ""

    # ── REQUIREMENTSDATEFORCOMPONENT ──────────
    def validate_requirementsdateforcomponent(self, row) -> str:
        val = row.get("REQUIREMENTSDATEFORCOMPONENT", "")
        if self._is_blank(val):
            return "REQUIREMENTSDATEFORCOMPONENT: Field is blank"
        if not self._valid_date(val):
            return (
                f"REQUIREMENTSDATEFORCOMPONENT: '{str(val).strip()}' does not follow "
                "the required format YYYYMMDD"
            )
        return ""

    # ── REMAININGQTY ──────────────────────────
    def validate_remainingqty(self, row) -> str:
        if self._is_blank(row.get("REMAININGQTY")):
            return "REMAININGQTY: Field is blank"
        return ""

    # ── REQUIREMENTQUANTITYINBUOM ─────────────
    def validate_requirementquantityinbuom(self, row) -> str:
        if self._is_blank(row.get("REQUIREMENTQUANTITYINBUOM")):
            return "REQUIREMENTQUANTITYINBUOM: Field is blank"
        return ""

    def get_rules(self) -> dict:
        return {
            "ORDERNUMBER":                  self.validate_ordernumber,
            "PLANT":                        self.validate_plant,
            "BOMITEMNUMBER":                self.validate_bomitemnumber,
            "MATERIALNUMBER":               self.validate_materialnumber,
            "REQUIREMENTSDATEFORCOMPONENT": self.validate_requirementsdateforcomponent,
            "REMAININGQTY":                 self.validate_remainingqty,
            "REQUIREMENTQUANTITYINBUOM":    self.validate_requirementquantityinbuom,
        }
...
#  Validator
# ══════════════════════════════════════════════
class AllocationTableValidator:

    def __init__(self, allocation_path: str, partfg_path: str, partrm_path: str, site_path: str):
        self.allocation_path   = allocation_path
        self.partfg_path       = partfg_path
        self.partrm_path       = partrm_path
        self.site_path         = site_path
        self.df                = pd.DataFrame()
        self.part_fg_materials = set()
        self.part_rm_materials = set()
        self.site_plants       = set()
        self.error_map         = {}
        self.reason_map        = {}

    def load(self):
        self.df = pd.read_csv(self.allocation_path, sep="\t", dtype=str)
        ...
        rm_df = pd.read_excel(self.partrm_path, dtype=str, engine="openpyxl")
        rm_df.columns = [c.strip().upper() for c in rm_df.columns]
        if "MATERIALNUMBER" not in rm_df.columns:
            raise ValueError("MATERIALNUMBER column not found in Part(RM) master.")
        self.part_rm_materials = set(rm_df["MATERIALNUMBER"].dropna().str.strip().str.upper().tolist())
        print(f"    Part(RM)  materials loaded : {len(self.part_rm_materials)} unique values")
    ...
    def validate(self):
        engine = AllocationRuleEngine(self.part_fg_materials, self.part_rm_materials, self.site_plants)
    ...
    def get_requirementsdate_error_subcounts(self) -> dict:
        counts = {"blank": 0, "bad_format": 0}
        for idx, col_reason in self.reason_map.items():
            reason = col_reason.get("REQUIREMENTSDATEFORCOMPONENT", "")
            if not reason:
                continue
            if "blank" in reason.lower():
                counts["blank"] += 1
...
class AllocationReportWriter:
    ...
    RULES_CONTENT = {
        ...
        "MATERIALNUMBER": [
            "Must not be blank.",
            "Must be present in the Part(FG) master OR Part(RM) master (MATERIALNUMBER column).",
        ],
        "REQUIREMENTSDATEFORCOMPONENT": [
            "Must not be blank.",
    ...
    def __init__(self, validator: AllocationTableValidator, output_path: str):
    ...
    def _write_summary_sheet_into(self, ws, error_map: dict, total_rows: int):
        ...
        title_cell           = ws.cell(row=1, column=1, value="Allocation Validation Summary")
        ...
        reqdate_subcounts = self.validator.get_requirementsdate_error_subcounts()
        ...
        for col_name in FIELD_ORDER:
            ...
            if col_name == "MATERIALNUMBER" and has_errors:
                for sub_label, sub_count, sub_reason in [
                    ...
                    ("  ↳ Not in Part(FG) or Part(RM) Master", mat_subcounts["not_in_master"],
                     "MATERIALNUMBER: Not present in Part(FG) or Part(RM) master"),
                ]:
                    sub_pct_err    = round((sub_count / total_rows) * 100, 2) if total_rows else 0
                    sub_pct_health = round(100 - sub_pct_err, 2)
                    ws.cell(row=row_num, column=1, value="")
            ...
            # ── REQUIREMENTSDATEFORCOMPONENT sub-rows ──
            if col_name == "REQUIREMENTSDATEFORCOMPONENT" and has_errors:
                for sub_label, sub_count, sub_reason in [
                    ("  ↳ Blank Requirements Date",       reqdate_subcounts["blank"],
                     "REQUIREMENTSDATEFORCOMPONENT: Field is blank"),
                    ("  ↳ Invalid Format (not YYYYMMDD)", reqdate_subcounts["bad_format"],
                     "REQUIREMENTSDATEFORCOMPONENT: Does not follow required format YYYYMMDD"),
                ]:
                    sub_pct_err    = round((sub_count / total_rows) * 100, 2) if total_rows else 0
                    sub_pct_health = round(100 - sub_pct_err, 2)
                    ws.cell(row=row_num, column=1, value="")
    ...
    def _write_rules_sheet(self, wb):
        ...
        title_cell           = ws.cell(row=1, column=1, value="Allocation Table – Validation Rules")
...
#  Orchestrator
# ══════════════════════════════════════════════
class AllocationTableProcessor:

    def __init__(self, allocation_path: str, partfg_path: str, partrm_path: str,
                 site_path: str, output_path: str):
        self.validator = AllocationTableValidator(allocation_path, partfg_path, partrm_path, site_path)
        self.writer    = AllocationReportWriter(self.validator, output_path)

    def run(self):
        print("📂  Loading files …")
        self.validator.load()
        print(f"    Allocation columns detected : {list(self.validator.df.columns)}")
...
if __name__ == "__main__":
    processor = AllocationTableProcessor(
        allocation_path = ALLOCATION_INPUT_FILE,
        partfg_path     = PARTFG_INPUT_FILE,
        partrm_path     = PARTRM_INPUT_FILE,
