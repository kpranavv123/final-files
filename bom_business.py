...
OUTPUT_FILE    = r"C:\Users\SW526XH\Downloads\Go Live-2\BOM\Validated_BOM_Business.xlsx"
...
# ─────────────────────────────────────────────
FIELD_ORDER = ["VALIDFROM", "VALIDTO", "BASEQUANTITY"]
...
#  Rule Engine
# ══════════════════════════════════════════════
class BOMBusinessRuleEngine:
    """
    Rules:
      VALIDFROM
        a) VALIDFROM must not be greater than VALIDTO.
      VALIDTO
    ...
    def validate_row(self, row) -> dict:
        """Returns {field_name: reason} for whichever of VALIDFROM /
        VALIDTO / BASEQUANTITY failed a business rule on this row."""
        reasons = {}

        # ── VALIDFROM / VALIDTO: date order check ──
        vf_raw = row.get("VALIDFROM", "")
        vt_raw = row.get("VALIDTO", "")
        vf_blank = self._is_blank(vf_raw)
        vt_blank = self._is_blank(vt_raw)

        if not vf_blank and not vt_blank:
            vf_date = self._parse_date(vf_raw)
            vt_date = self._parse_date(vt_raw)
            if vf_date and vt_date and vf_date > vt_date:
                reasons["VALIDFROM"] = (
                    f"VALIDFROM: '{str(vf_raw).strip()}' is greater than "
                    f"VALIDTO '{str(vt_raw).strip()}'"
                )
                reasons["VALIDTO"] = (
                    f"VALIDTO: '{str(vt_raw).strip()}' is lesser than "
                    f"VALIDFROM '{str(vf_raw).strip()}'"
                )

        # ── BASEQUANTITY: negative value check ──
        bq_raw = row.get("BASEQUANTITY", "")
        bq_num = self._parse_number(bq_raw)
        if bq_num is not None and bq_num < 0:
            reasons["BASEQUANTITY"] = (
                f"BASEQUANTITY: '{str(bq_raw).strip()}' is a negative value"
            )
...
#  Validator
# ══════════════════════════════════════════════
class BOMBusinessTableValidator:
...
class BOMBusinessReportWriter:
    ...
    RULES_CONTENT = {
        "VALIDFROM": [
            "VALIDFROM must not be greater than VALIDTO.",
        ],
        "VALIDTO": [
            "VALIDTO must not be lesser than VALIDFROM.",
        ],
        "BASEQUANTITY": [
            "Negative values are not allowed.",
        ],
    ...
    def __init__(self, validator: BOMBusinessTableValidator, output_path: str):
    ...
    def _write_summary_sheet_into(self, ws, error_map: dict, total_rows: int):
        ...
        title_cell           = ws.cell(row=1, column=1, value="BOM Business Validation Summary")
    ...
    def _write_field_error_sheets(self, wb, df: pd.DataFrame, all_cols: list):
        ...
        for field_name in FIELD_ORDER:
            ...
            for excel_row, (orig_idx, row_data) in enumerate(subset.iterrows(), start=2):
                ...
                # Highlight every field involved in this row's failure
    ...
    def _write_rules_sheet(self, wb):
        ...
        title_cell           = ws.cell(row=1, column=1, value="BOM Table – Business Validation Rules")
...
#  Orchestrator
# ══════════════════════════════════════════════
class BOMBusinessTableProcessor:

    def __init__(self, bom_path: str, output_path: str):
        self.validator = BOMBusinessTableValidator(bom_path)
        self.writer    = BOMBusinessReportWriter(self.validator, output_path)
...
if __name__ == "__main__":
    processor = BOMBusinessTableProcessor(
