...
#  FILE PATHS
# ─────────────────────────────────────────────
WORKCENTER_INPUT_FILE = r"C:\Users\SW526XH\Downloads\Go Live-2\Workcenter\Workcenter.tab"
SITE_INPUT_FILE       = r"C:\Users\SW526XH\Downloads\Go Live-2\Workcenter\Site_2026-04-09-1058.xlsx"
OUTPUT_FILE           = r"C:\Users\SW526XH\Downloads\Go Live-2\Workcenter\Validated_Workcenter.xlsx"
...
# ─────────────────────────────────────────────
KEEP_COLS = [
    "WORKCENTER", "PLANT", "SHORTDESCRIPTIONINCAPITALS",
    "PERSONRESPONSIBLE", "WORKCENTERCATEGORY", "CAPACITYBASEUNITOFMEASURE",
]
...
FIELDS_WITH_SUB_ROWS = {"PLANT"}

# Per-field single-line reason shown in summary (blank for sub-row fields)
FIELD_REASON = {
    "WORKCENTER":                  "WORKCENTER: Field is blank",
    "PLANT":                       "",   # sub-rows carry reasons
    "SHORTDESCRIPTIONINCAPITALS":  "SHORTDESCRIPTIONINCAPITALS: Field is blank",
    "PERSONRESPONSIBLE":           "PERSONRESPONSIBLE: Field is blank",
    "WORKCENTERCATEGORY":          "WORKCENTERCATEGORY: Field is blank",
    "CAPACITYBASEUNITOFMEASURE":   "CAPACITYBASEUNITOFMEASURE: Field is blank",
}
...
class WorkcenterRuleEngine:
    ...
    def validate_workcenter(self, row) -> str:
        if self._is_blank(row.get("WORKCENTER")):
    ...
    # ── SHORTDESCRIPTIONINCAPITALS ────────────
    def validate_shortdescriptionincapitals(self, row) -> str:
        if self._is_blank(row.get("SHORTDESCRIPTIONINCAPITALS")):
            return "SHORTDESCRIPTIONINCAPITALS: Field is blank"
        return ""

    # ── PERSONRESPONSIBLE ─────────────────────
    def validate_personresponsible(self, row) -> str:
        if self._is_blank(row.get("PERSONRESPONSIBLE")):
            return "PERSONRESPONSIBLE: Field is blank"
        return ""

    # ── WORKCENTERCATEGORY ────────────────────
    def validate_workcentercategory(self, row) -> str:
        if self._is_blank(row.get("WORKCENTERCATEGORY")):
            return "WORKCENTERCATEGORY: Field is blank"
        return ""

    # ── CAPACITYBASEUNITOFMEASURE ─────────────
    def validate_capacitybaseunitofmeasure(self, row) -> str:
        if self._is_blank(row.get("CAPACITYBASEUNITOFMEASURE")):
            return "CAPACITYBASEUNITOFMEASURE: Field is blank"
        return ""
    ...
    def get_rules(self) -> dict:
        return {
            ...
            "SHORTDESCRIPTIONINCAPITALS": self.validate_shortdescriptionincapitals,
            "PERSONRESPONSIBLE":          self.validate_personresponsible,
            "WORKCENTERCATEGORY":         self.validate_workcentercategory,
            "CAPACITYBASEUNITOFMEASURE":  self.validate_capacitybaseunitofmeasure,
        }
...
#  Validator
# ══════════════════════════════════════════════
class WorkcenterTableValidator:

    def __init__(self, workcenter_path: str, site_path: str):
        self.workcenter_path = workcenter_path
    ...
    def load(self):
        self.df = pd.read_csv(self.workcenter_path, sep="\t", dtype=str)
    ...
    def validate(self):
        engine = WorkcenterRuleEngine(self.site_plants)
...
class WorkcenterReportWriter:
    ...
    def __init__(self, validator: WorkcenterTableValidator, output_path: str):
    ...
    def _write_summary_sheet_into(self, ws, error_map: dict, total_rows: int):
        ...
        title_cell           = ws.cell(row=1, column=1, value="Workcenter Validation Summary")
    ...
    def _write_rules_sheet(self, wb):
        ...
        title_cell           = ws.cell(row=1, column=1, value="Workcenter Table – Validation Rules")
...
#  Orchestrator
# ══════════════════════════════════════════════
class WorkcenterTableProcessor:

    def __init__(self, workcenter_path: str, site_path: str, output_path: str):
        self.validator = WorkcenterTableValidator(workcenter_path, site_path)
        self.writer    = WorkcenterReportWriter(self.validator, output_path)

    def run(self):
        print("📂  Loading files …")
        self.validator.load()
        print(f"    Workcenter columns detected : {list(self.validator.df.columns)}")
...
if __name__ == "__main__":
    processor = WorkcenterTableProcessor(
        workcenter_path = WORKCENTER_INPUT_FILE,
