...
# File paths
# ====================================================
HDA_FILE      = r"C:\Users\SW526XH\Downloads\Go Live-1\HDA_Secondary\HistoricalDemandActual_DistributorStock.tab"
...
OUTPUT_EXCEL  = r"C:\Users\SW526XH\Downloads\Go Live-1\HDA_Secondary\Validated_Distributor_Stock_Technical_Updated.xlsx"
...
DUPLICATE_KEY_COLS = ["SOLDTOPARTY", "PLANT", "CLOSINGSTOCKDATE", "MATERIAL"]
...
ERROR_REASON_MAP = {
    "ERROR_MATERIAL":         "MATERIAL: MATERIAL missing in Part master",
    "ERROR_SOLDTOPARTY":      "SOLDTOPARTY: Soldtoparty is Blank",
    "ERROR_CLOSINGSTOCKDATE": "CLOSINGSTOCKDATE: Closing stock date is blank or not in YYYYMMDD format",
    "ERROR_PLANT":            "PLANT: Plant does not exist in Site master or is blank",
    "ERROR_DUPLICATE":        "DUPLICATE_CHECK: Duplicate record — SOLDTOPARTY, PLANT, CLOSINGSTOCKDATE, MATERIAL combination already exists.",
}

ERROR_COLUMNS = list(ERROR_REASON_MAP.keys())

SUMMARY_RULES = [
    ("MATERIAL",          "ERROR_MATERIAL",         ERROR_REASON_MAP["ERROR_MATERIAL"]),
    ("SOLDTOPARTY",       "ERROR_SOLDTOPARTY",      ERROR_REASON_MAP["ERROR_SOLDTOPARTY"]),
    ("CLOSINGSTOCKDATE",  "ERROR_CLOSINGSTOCKDATE", ERROR_REASON_MAP["ERROR_CLOSINGSTOCKDATE"]),
    ("PLANT",             "ERROR_PLANT",            ERROR_REASON_MAP["ERROR_PLANT"]),
    ("DUPLICATE_CHECK",   "ERROR_DUPLICATE",        ERROR_REASON_MAP["ERROR_DUPLICATE"]),
]

# FIX: key was "CLOSING_CLOSINGSTOCKDATE" — didn't match the field name used
# everywhere else (SUMMARY_RULES / ATTRIBUTE_SHEETS / attribute_sheet_rows all
# use "CLOSINGSTOCKDATE"), so the Rule_Set sheet showed a mismatched field name.
RULESET_DESCRIPTIONS = {
    "MATERIAL":            ["Must not be blank.", "Must exist as MATERIALNUMBER in Part master."],
    "SOLDTOPARTY": [
        "Must not be blank.",
        # "SOLDTOPARTY + PLANT combination must exist as CUSTOMER + SUPPLYINGPLANT in Customer master.",
    ],
    "CLOSINGSTOCKDATE":    ["Must not be blank.", "Must strictly be in YYYYMMDD format."],
    "PLANT":               ["Must not be blank.", "Must exist in Site master."],
    "DUPLICATE_CHECK":  [
        "Must not be blank.",
        "No duplicate combinations of SOLDTOPARTY + PLANT + CLOSINGSTOCKDATE + MATERIAL allowed.",
        "All occurrences of a duplicate group are flagged as errors.",
    ],
}

ATTRIBUTE_SHEETS = {
    "ERROR_MATERIAL":         ("MATERIAL",          ERROR_REASON_MAP["ERROR_MATERIAL"]),
    "ERROR_SOLDTOPARTY":      ("SOLDTOPARTY",       ERROR_REASON_MAP["ERROR_SOLDTOPARTY"]),
    "ERROR_CLOSINGSTOCKDATE": ("CLOSINGSTOCKDATE",  ERROR_REASON_MAP["ERROR_CLOSINGSTOCKDATE"]),
...
attribute_sheet_rows = {
    ...
    for base in ["MATERIAL", "SOLDTOPARTY", "CLOSINGSTOCKDATE", "PLANT", "DUPLICATE_CHECK"]
}
...
# ====================================================
print("📂 Loading full Distributor Stock file for validation...")
...
#     "CLOSING_STOCKS_DATE": "INVOICE_DATE",
#     "SITE": "PLANT"
# }

# hda_df.rename(columns=COLUMN_MAPPING, inplace=True)
...
hda_df["ERROR_MATERIAL"] = hda_df["MATERIAL"].apply(
...
#     axis=1,
# )
hda_df["ERROR_SOLDTOPARTY"] = hda_df["SOLDTOPARTY"].apply(
    lambda x: "Yes" if pd.isna(x) or str(x).strip() == "" else ""
)

hda_df["ERROR_CLOSINGSTOCKDATE"] = hda_df["CLOSINGSTOCKDATE"].apply(
...
# (SOLDTOPARTY + PLANT + CLOSINGSTOCKDATE + MATERIAL)
...
with pd.ExcelWriter(OUTPUT_EXCEL, engine="openpyxl") as writer:
    # FIX: `sheet_created` tracking and the "No_Errors" fallback were missing
    # from this version entirely. If every error count comes back 0, the loop
    # below never writes a single sheet, and the workbook closes with zero
    # sheets -> "IndexError: At least one sheet must be visible" on save.
    sheet_created = False
...
title_cell           = ws_sum.cell(row=1, column=1, value="Distributor Stock Validation Summary")
...
title_cell           = wsr.cell(row=1, column=1, value="Distributor Stock – Validation Rules")
