...
#  FILE PATHS
# ─────────────────────────────────────────────
HSAMAKE_INPUT_FILE = r"C:\Users\SW526XH\Downloads\Go Live-2\HSA_Make\HSA_Make.tab"
PARTFG_INPUT_FILE  = r"C:\Users\SW526XH\Downloads\Go Live-2\HSA_Make\Part_FG.xlsx"
SITE_INPUT_FILE    = r"C:\Users\SW526XH\Downloads\Go Live-2\HSA_Make\Site_2026-04-09-1058.xlsx"
OUTPUT_FILE        = r"C:\Users\SW526XH\Downloads\Go Live-2\HSA_Make\Validated_HSA_Make.xlsx"
...
FIELD_REASON = {
    ...
    "MFGORDERITEMACTUALDELIVERYDATE":  "",   # sub-rows carry reasons
    "QUANTITY":                        "QUANTITY: Field is blank",
    "ACTUALORDERFINISHDATE":           "",   # sub-rows carry reasons
}
...
#  Rule Engine
# ══════════════════════════════════════════════
class HSAMakeRuleEngine:

    def __init__(self, part_fg_materials: set, site_plants: set):
    ...
    def validate_partname(self, row) -> str:
        ...
        if val_str not in self.part_fg_materials:
            return f"PARTNAME: '{val_str}' is not present in Part(FG) master"
        return ""
    ...
    # ── MFGORDERITEMACTUALDELIVERYDATE ────────
    def validate_mfgorderitemactualdeliverydate(self, row) -> str:
        val = row.get("MFGORDERITEMACTUALDELIVERYDATE", "")
        if self._is_blank(val):
            return "MFGORDERITEMACTUALDELIVERYDATE: Field is blank"
        if not self._valid_date(val):
            return (
                f"MFGORDERITEMACTUALDELIVERYDATE: '{str(val).strip()}' "
                f"does not follow the required format YYYYMMDD"
            )
        return ""

    # ── QUANTITY ──────────────────────────────
    def validate_quantity(self, row) -> str:
        if self._is_blank(row.get("QUANTITY")):
            return "QUANTITY: Field is blank"
        return ""

    # ── ACTUALORDERFINISHDATE ─────────────────
    def validate_actualorderfinishdate(self, row) -> str:
        val = row.get("ACTUALORDERFINISHDATE", "")
        if self._is_blank(val):
            return "ACTUALORDERFINISHDATE: Field is blank"
        if not self._valid_date(val):
            return (
                f"ACTUALORDERFINISHDATE: '{str(val).strip()}' "
                f"does not follow the required format YYYYMMDD"
            )
        return ""
    ...
    def get_rules(self) -> dict:
        return {
            ...
            "MFGORDERITEMACTUALDELIVERYDATE": self.validate_mfgorderitemactualdeliverydate,
            "QUANTITY":                       self.validate_quantity,
            "ACTUALORDERFINISHDATE":          self.validate_actualorderfinishdate,
        }
...
#  Validator
# ══════════════════════════════════════════════
class HSAMakeTableValidator:

    def __init__(self, hsamake_path: str, partfg_path: str, site_path: str):
        self.hsamake_path      = hsamake_path
    ...
    def load(self):
        self.df = pd.read_csv(self.hsamake_path, sep="\t", dtype=str)
    ...
    def validate(self):
        engine = HSAMakeRuleEngine(self.part_fg_materials, self.site_plants)
    ...
    def get_ordersuppliersite_error_subcounts(self) -> dict:
        ...
        for idx, col_reason in self.reason_map.items():
            reason = col_reason.get("ORDERSUPPLIERSITE", "")
            if not reason:
                continue
            if "blank" in reason.lower():
                counts["blank"] += 1
    ...
    def get_mfgorderitemactualdeliverydate_error_subcounts(self) -> dict:
        counts = {"blank": 0, "bad_format": 0}
        for idx, col_reason in self.reason_map.items():
            reason = col_reason.get("MFGORDERITEMACTUALDELIVERYDATE", "")
            if not reason:
                continue
            if "blank" in reason.lower():
                counts["blank"] += 1
    ...
    def get_actualorderfinishdate_error_subcounts(self) -> dict:
        counts = {"blank": 0, "bad_format": 0}
        for idx, col_reason in self.reason_map.items():
            reason = col_reason.get("ACTUALORDERFINISHDATE", "")
            if not reason:
                continue
            if "blank" in reason.lower():
                counts["blank"] += 1
...
class HSAMakeReportWriter:
    ...
    def __init__(self, validator: HSAMakeTableValidator, output_path: str):
    ...
    def _write_summary_sheet_into(self, ws, error_map: dict, total_rows: int):
        ...
        title_cell           = ws.cell(row=1, column=1, value="HSA (Make) Validation Summary")
        ...
        partname_sub   = self.validator.get_partname_error_subcounts()
        oss_sub        = self.validator.get_ordersuppliersite_error_subcounts()
        mfg_sub        = self.validator.get_mfgorderitemactualdeliverydate_error_subcounts()
        aofd_sub       = self.validator.get_actualorderfinishdate_error_subcounts()
        ...
        for col_name in FIELD_ORDER:
            ...
            # ── PARTNAME sub-rows ──
            if col_name == "PARTNAME" and has_errors:
                sub_definitions = [
                    (
                        "  ↳ Blank Part Name",
                        partname_sub["blank"],
            ...
            # ── ORDERSUPPLIERSITE sub-rows ──
            if col_name == "ORDERSUPPLIERSITE" and has_errors:
                sub_definitions = [
                    (
                        "  ↳ Blank Order Supplier Site",
                        oss_sub["blank"],
                    ...
                    (
                        "  ↳ Not in Site Master",
                        oss_sub["not_in_site"],
                        "ORDERSUPPLIERSITE: Site not found in the Site master",
                    ),
                ]
                for sub_label, sub_count, sub_reason in sub_definitions:
                    sub_pct_err    = round((sub_count / total_rows) * 100, 2) if total_rows else 0
            ...
            # ── MFGORDERITEMACTUALDELIVERYDATE sub-rows ──
            if col_name == "MFGORDERITEMACTUALDELIVERYDATE" and has_errors:
                sub_definitions = [
                    (
                        "  ↳ Blank Mfg Order Item Actual Delivery Date",
                        mfg_sub["blank"],
                        "MFGORDERITEMACTUALDELIVERYDATE: Field is blank",
                    ),
                    (
                        "  ↳ Invalid Format (not YYYYMMDD)",
                        mfg_sub["bad_format"],
                        "MFGORDERITEMACTUALDELIVERYDATE: Does not follow required format YYYYMMDD",
                    ),
                ]
                for sub_label, sub_count, sub_reason in sub_definitions:
                    sub_pct_err    = round((sub_count / total_rows) * 100, 2) if total_rows else 0
            ...
            # ── ACTUALORDERFINISHDATE sub-rows ──
            if col_name == "ACTUALORDERFINISHDATE" and has_errors:
                sub_definitions = [
                    (
                        "  ↳ Blank Actual Order Finish Date",
                        aofd_sub["blank"],
                        "ACTUALORDERFINISHDATE: Field is blank",
                    ),
                    (
                        "  ↳ Invalid Format (not YYYYMMDD)",
                        aofd_sub["bad_format"],
                        "ACTUALORDERFINISHDATE: Does not follow required format YYYYMMDD",
                    ),
                ]
                for sub_label, sub_count, sub_reason in sub_definitions:
                    sub_pct_err    = round((sub_count / total_rows) * 100, 2) if total_rows else 0
    ...
    def _write_rules_sheet(self, wb):
        ...
        title_cell           = ws.cell(row=1, column=1, value="HSA (Make) Table – Validation Rules")
...
#  Orchestrator
# ══════════════════════════════════════════════
class HSAMakeTableProcessor:

    def __init__(self, hsamake_path: str, partfg_path: str,
                 site_path: str, output_path: str):
        self.validator = HSAMakeTableValidator(hsamake_path, partfg_path, site_path)
        self.writer    = HSAMakeReportWriter(self.validator, output_path)

    def run(self):
        print("📂  Loading files …")
        self.validator.load()
        print(f"    HSA(Make) columns detected : {list(self.validator.df.columns)}")
...
if __name__ == "__main__":
    processor = HSAMakeTableProcessor(
        hsamake_path = HSAMAKE_INPUT_FILE,
