...
#  FILE PATHS
# ─────────────────────────────────────────────
HSABUY_INPUT_FILE  = r"C:\Users\SW526XH\Downloads\Go Live-2\HSA_Buy\HSA_Buy.tab"
PARTFG_INPUT_FILE  = r"C:\Users\SW526XH\Downloads\Go Live-2\HSA_Buy\Part_FG.xlsx"
SITE_INPUT_FILE    = r"C:\Users\SW526XH\Downloads\Go Live-2\HSA_Buy\Site_2026-04-09-1058.xlsx"
OUTPUT_FILE        = r"C:\Users\SW526XH\Downloads\Go Live-2\HSA_Buy\Validated_HSA_Buy.xlsx"
...
FIELD_REASON = {
    ...
    "SOURCEPLANT":                 "",   # sub-rows carry reasons
    "VENDORACCOUNTNUMBER":         "VENDORACCOUNTNUMBER: Field is blank",
    "TOTALRECEIVEDQUANTITYINBU":   "TOTALRECEIVEDQUANTITYINBU: Field is blank",
    "LATESTDOCKDATE":              "",   # sub-rows carry reasons
}
...
class HSABuyRuleEngine:
    ...
    def validate_materialnumber(self, row) -> str:
        ...
        if val_str not in self.part_fg_materials:
            return f"MATERIALNUMBER: '{val_str}' is not present in Part(FG) master"
        return ""

    # ── SOURCEPLANT ───────────────────────────
    def validate_sourceplant(self, row) -> str:
        val = str(row.get("SOURCEPLANT", "")).strip()
        if not val or val == "nan":
            return "SOURCEPLANT: Field is blank"
        if val not in self.site_plants:
            return f"SOURCEPLANT: '{val}' is not present in the Site master"
        return ""

    # ── VENDORACCOUNTNUMBER ───────────────────
    def validate_vendoraccountnumber(self, row) -> str:
        if self._is_blank(row.get("VENDORACCOUNTNUMBER")):
            return "VENDORACCOUNTNUMBER: Field is blank"
        return ""

    # ── TOTALRECEIVEDQUANTITYINBU ─────────────
    def validate_totalreceivedquantityinbu(self, row) -> str:
        if self._is_blank(row.get("TOTALRECEIVEDQUANTITYINBU")):
            return "TOTALRECEIVEDQUANTITYINBU: Field is blank"
        return ""

    # ── LATESTDOCKDATE ─────────────────────────
    def validate_latestdockdate(self, row) -> str:
        val = row.get("LATESTDOCKDATE", "")
        if self._is_blank(val):
            return "LATESTDOCKDATE: Field is blank"
        if not self._valid_date(val):
            return (
                f"LATESTDOCKDATE: '{str(val).strip()}' "
    ...
    def get_rules(self) -> dict:
        return {
            ...
            "VENDORACCOUNTNUMBER":        self.validate_vendoraccountnumber,
            "TOTALRECEIVEDQUANTITYINBU":  self.validate_totalreceivedquantityinbu,
            "LATESTDOCKDATE":             self.validate_latestdockdate,
        }
...
#  Validator
# ══════════════════════════════════════════════
class HSABuyTableValidator:

    def __init__(self, hsabuy_path: str, partfg_path: str, site_path: str):
        self.hsabuy_path       = hsabuy_path
    ...
    def load(self):
        self.df = pd.read_csv(self.hsabuy_path, sep="\t", dtype=str)
    ...
    def validate(self):
        engine = HSABuyRuleEngine(self.part_fg_materials, self.site_plants)
    ...
    def get_sourceplant_error_subcounts(self) -> dict:
        counts = {"blank": 0, "not_in_site": 0}
        for idx, col_reason in self.reason_map.items():
            reason = col_reason.get("SOURCEPLANT", "")
            if not reason:
                continue
            if "blank" in reason.lower():
                counts["blank"] += 1
    ...
    def get_latestdockdate_error_subcounts(self) -> dict:
        counts = {"blank": 0, "bad_format": 0}
        for idx, col_reason in self.reason_map.items():
            reason = col_reason.get("LATESTDOCKDATE", "")
            if not reason:
                continue
            if "blank" in reason.lower():
                counts["blank"] += 1
...
class HSABuyReportWriter:
    ...
    def __init__(self, validator: HSABuyTableValidator, output_path: str):
    ...
    def _write_summary_sheet_into(self, ws, error_map: dict, total_rows: int):
        ...
        title_cell           = ws.cell(row=1, column=1, value="HSA (Buy) Validation Summary")
        ...
        srcpl_sub   = self.validator.get_sourceplant_error_subcounts()
        ldd_sub     = self.validator.get_latestdockdate_error_subcounts()
        ...
        for col_name in FIELD_ORDER:
            ...
            # ── SOURCEPLANT sub-rows ──
            if col_name == "SOURCEPLANT" and has_errors:
                sub_definitions = [
                    (
                        "  ↳ Blank Source Plant",
                        srcpl_sub["blank"],
                        "SOURCEPLANT: Field is blank",
                    ),
                    (
                        "  ↳ Not in Site Master",
                        srcpl_sub["not_in_site"],
                        "SOURCEPLANT: Site not found in the Site master",
                    ),
                ]
                for sub_label, sub_count, sub_reason in sub_definitions:
                    sub_pct_err    = round((sub_count / total_rows) * 100, 2) if total_rows else 0
            ...
            # ── LATESTDOCKDATE sub-rows ──
            if col_name == "LATESTDOCKDATE" and has_errors:
                sub_definitions = [
                    (
                        "  ↳ Blank Latest Dock Date",
                        ldd_sub["blank"],
                        "LATESTDOCKDATE: Field is blank",
                    ),
                    (
                        "  ↳ Invalid Format (not YYYYMMDD)",
                        ldd_sub["bad_format"],
                        "LATESTDOCKDATE: Does not follow required format YYYYMMDD",
                    ),
                ]
                for sub_label, sub_count, sub_reason in sub_definitions:
                    sub_pct_err    = round((sub_count / total_rows) * 100, 2) if total_rows else 0
    ...
    def _write_rules_sheet(self, wb):
        ...
        title_cell           = ws.cell(row=1, column=1, value="HSA (Buy) Table – Validation Rules")
...
#  Orchestrator
# ══════════════════════════════════════════════
class HSABuyTableProcessor:

    def __init__(self, hsabuy_path: str, partfg_path: str,
                 site_path: str, output_path: str):
        self.validator = HSABuyTableValidator(hsabuy_path, partfg_path, site_path)
        self.writer    = HSABuyReportWriter(self.validator, output_path)

    def run(self):
        print("📂  Loading files …")
        self.validator.load()
        print(f"    HSA(Buy) columns detected : {list(self.validator.df.columns)}")
...
if __name__ == "__main__":
    processor = HSABuyTableProcessor(
        hsabuy_path = HSABUY_INPUT_FILE,
