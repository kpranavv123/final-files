"""
HDA (Make) Business Validator
═════════════════════════════════════════════════════════════════════════════

Purpose:
  Validates HDA (Make) file against business rules only.

Business Rules:
  - QUANTITY should not contain negative values

Output:
  - Validated_HDAMake_Business.xlsx
"""
...
# ─────────────────────────────────────────────
HDA_MAKE_INPUT = r"C:\Users\SW526XH\Downloads\Go Live-2\HDA(Make)\HDAMake_2026-07-06-1156.tab"
OUTPUT_FILE    = r"C:\Users\SW526XH\Downloads\Go Live-2\HDA(Make)\Validated_HDAMake_Business.xlsx"
...
BUSINESS_RULESET_INFO = {
    "QUANTITY": [
        "QUANTITY contains negative value",
    ],
}
...
#  Rule Engine
# ══════════════════════════════════════════════
class HDAMakeBusinessRuleEngine:
    ...
    def validate_quantity_negative(self, row) -> tuple:
        ...
        try:
            if float(val) < 0:
                return False, "QUANTITY contains negative value"
    ...
    def get_rules(self) -> dict:
        return {
            "QUANTITY": self.validate_quantity_negative,
        }
...
#  Validator
# ══════════════════════════════════════════════
class HDAMakeBusinessValidator:
    ...
    def load(self):
        ...
        if HDA_MAKE_INPUT.lower().endswith(".xlsx"):
            self.df = pd.read_excel(HDA_MAKE_INPUT, dtype=str)
        else:
            self.df = pd.read_csv(HDA_MAKE_INPUT, sep="\t", dtype=str)

        print(f"    HDA (Make) - rows loaded: {len(self.df)}")
    ...
    def validate(self):
        ...
        engine = HDAMakeBusinessRuleEngine()
...
#  Report Writer
# ══════════════════════════════════════════════
class HDAMakeBusinessReportWriter:
    SHEET_SUMMARY  = "Summary"
    SHEET_RULESETS = "Rulesets"

    def __init__(self, validator: HDAMakeBusinessValidator, output_path: str):
    ...
    def _write_ruleset_sheet(self, wb, summary_fields=None):
        ...
        title_cell           = ws.cell(
            row=1, column=1,
            value="HDA (Make) - Business Validation Rules",
        )
    ...
    def _write_summary_sheet(self, wb, error_map: dict, total_rows: int):
        ...
        title_cell = ws.cell(
            row=1, column=1,
            value="HDA (Make) Business Rules Summary",
        )
        ...
        for field_num, (col_name, count) in enumerate(sorted_fields, start=1):
            ...
            canonical_reasons = {
                "QUANTITY": "QUANTITY contains negative value",
            }
...
#  Orchestrator
# ══════════════════════════════════════════════
class HDAMakeBusinessProcessor:
    def __init__(self):
        self.validator = HDAMakeBusinessValidator()
        self.writer    = HDAMakeBusinessReportWriter(self.validator, OUTPUT_FILE)

    def run(self):
        print("=" * 70)
        print("HDA (Make) Business Validation")
        print("=" * 70)
