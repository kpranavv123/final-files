...
#  FILE PATHS
# ─────────────────────────────────────────────
PARTSOURCEBUY_INPUT_FILE = r"C:\Users\SW526XH\Downloads\Go Live-2\PartSource_Buy\PartSource_Buy.tab"
PARTFG_INPUT_FILE        = r"C:\Users\SW526XH\Downloads\Go Live-2\PartSource_Buy\Part_FG.xlsx"
SITE_INPUT_FILE          = r"C:\Users\SW526XH\Downloads\Go Live-2\PartSource_Buy\Site_2026-04-09-1058.xlsx"
OUTPUT_FILE              = r"C:\Users\SW526XH\Downloads\Go Live-2\PartSource_Buy\Validated_PartSource_Buy.xlsx"
...
FIELD_REASON = {
    ...
    "PLANNEDDELIVERYTIME":     "PLANNEDDELIVERYTIME: Field is blank",
    "ORDERPOLICY":             "ORDERPOLICY: Field is blank",
    "SOURCEID":                "SOURCEID: Field is blank",
    "MAXIMUMPOQUANTITY":       "MAXIMUMPOQUANTITY: Field is blank",
    "MINIMUMPOQUANTITY":       "MINIMUMPOQUANTITY: Field is blank",
    "ROUNDINGVALUE":           "ROUNDINGVALUE: Field is blank",
    "GOODSRECEIPTDURATION":    "GOODSRECEIPTDURATION: Field is blank",
}
...
class PartSourceBuyRuleEngine:
    ...
    # ── PLANNEDDELIVERYTIME ────────────────────
    def validate_planneddeliverytime(self, row) -> str:
        if self._is_blank(row.get("PLANNEDDELIVERYTIME")):
            return "PLANNEDDELIVERYTIME: Field is blank"
        return ""

    # ── ORDERPOLICY ─────────────────────────────
    def validate_orderpolicy(self, row) -> str:
        if self._is_blank(row.get("ORDERPOLICY")):
            return "ORDERPOLICY: Field is blank"
        return ""

    # ── SOURCEID ─────────────────────────────────
    def validate_sourceid(self, row) -> str:
        if self._is_blank(row.get("SOURCEID")):
            return "SOURCEID: Field is blank"
        return ""

    # ── MAXIMUMPOQUANTITY ─────────────────────────
    def validate_maximumpoquantity(self, row) -> str:
        if self._is_blank(row.get("MAXIMUMPOQUANTITY")):
            return "MAXIMUMPOQUANTITY: Field is blank"
        return ""

    # ── MINIMUMPOQUANTITY ─────────────────────────
    def validate_minimumpoquantity(self, row) -> str:
        if self._is_blank(row.get("MINIMUMPOQUANTITY")):
            return "MINIMUMPOQUANTITY: Field is blank"
        return ""
    ...
    # ── GOODSRECEIPTDURATION ───────────────────────
    def validate_goodsreceiptduration(self, row) -> str:
        if self._is_blank(row.get("GOODSRECEIPTDURATION")):
            return "GOODSRECEIPTDURATION: Field is blank"
        return ""
    ...
    def get_rules(self) -> dict:
        return {
            ...
            "PLANNEDDELIVERYTIME":     self.validate_planneddeliverytime,
            "ORDERPOLICY":             self.validate_orderpolicy,
            "SOURCEID":                self.validate_sourceid,
            "MAXIMUMPOQUANTITY":       self.validate_maximumpoquantity,
            "MINIMUMPOQUANTITY":       self.validate_minimumpoquantity,
            "ROUNDINGVALUE":           self.validate_roundingvalue,
            "GOODSRECEIPTDURATION":    self.validate_goodsreceiptduration,
        }
...
#  Validator
# ══════════════════════════════════════════════
class PartSourceBuyTableValidator:

    def __init__(self, partsourcebuy_path: str, partfg_path: str, site_path: str):
        self.partsourcebuy_path = partsourcebuy_path
    ...
    def load(self):
        self.df = pd.read_csv(self.partsourcebuy_path, sep="\t", dtype=str)
    ...
    def validate(self):
        engine = PartSourceBuyRuleEngine(self.part_fg_materials, self.site_plants)
...
#  Report Writer
# ══════════════════════════════════════════════
class PartSourceBuyReportWriter:
    ...
    def __init__(self, validator: PartSourceBuyTableValidator, output_path: str):
    ...
    def _write_summary_sheet_into(self, ws, error_map: dict, total_rows: int):
        ...
        title_cell           = ws.cell(row=1, column=1, value="PartSource (Buy) Validation Summary")
    ...
    def _write_rules_sheet(self, wb):
        ...
        title_cell           = ws.cell(row=1, column=1, value="PartSource (Buy) Table – Validation Rules")
...
#  Orchestrator
# ══════════════════════════════════════════════
class PartSourceBuyTableProcessor:

    def __init__(self, partsourcebuy_path: str, partfg_path: str,
                 site_path: str, output_path: str):
        self.validator = PartSourceBuyTableValidator(partsourcebuy_path, partfg_path, site_path)
        self.writer    = PartSourceBuyReportWriter(self.validator, output_path)
...
if __name__ == "__main__":
    processor = PartSourceBuyTableProcessor(
        partsourcebuy_path = PARTSOURCEBUY_INPUT_FILE,
