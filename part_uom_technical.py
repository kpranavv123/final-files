...
class PartUOMTechnicalValidator:
    ...
    def load(self):
        ...
        # ✅ Load PartUOM input file
        uom_path = PART_UOM_INPUT_FILE.lower()

        if uom_path.endswith(".csv"):
          self.df = pd.read_csv(PART_UOM_INPUT_FILE, dtype=str)

        elif uom_path.endswith((".tab", ".tsv")):
         self.df = pd.read_csv(
        PART_UOM_INPUT_FILE,
        sep="\t",
        dtype=str,
        ...
        elif uom_path.endswith((".xlsx", ".xls")):
          self.df = pd.read_excel(PART_UOM_INPUT_FILE, dtype=str, engine="openpyxl")

        else:
          raise ValueError(f"Unsupported file format: {PART_UOM_INPUT_FILE}")


        ref_path = PART_REFERENCE_FILE.lower()

        if ref_path.endswith(".csv"):
         part_df = pd.read_csv(PART_REFERENCE_FILE, dtype=str)

        elif ref_path.endswith((".tab", ".tsv")):
         part_df = pd.read_csv(
         PART_REFERENCE_FILE,
         sep="\t",
         dtype=str,
...
class PartUOMTechnicalReportWriter:
    ...
    def _write_summary_sheet(self, wb, error_map: dict, total_rows: int):
        ...
        col_widths = [6, 28, 16, 16, 16, 16, 80]
