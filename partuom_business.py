"""
Part UOM Business Validator
═════════════════════════════════════════════════════════════════════════════

Purpose:
  Validates Part UOM Conversion file against business rules only.

Business Rules:
  ALTERNATIVEUNIT:
    1. All PRODUCT + PLANT combinations should have all standard alternate UOMs: KG, CV, PAC
    2. ALTERNATIVEUNIT should not be blank

Output:
  Validated_PartUOM_Business.xlsx
"""
...
#  FILE PATHS
# ─────────────────────────────────────────────
PART_UOM_INPUT_FILE = r"C:\Users\JE179KY\Downloads\Data_rulesets_check - Copy\Excel_Files\PartUOM.xlsx"
OUTPUT_FILE         = r"C:\Users\JE179KY\Downloads\Data_rulesets_check - Copy\Output_Files\Validated_PartUOM_Business.xlsx"


# Mandatory alternate UOMs per PRODUCT + PLANT
MANDATORY_UOMS = {"KG", "CV", "PAC"}
...
BUSINESS_RULESET_INFO = {
    "ALTERNATIVEUNIT": [
        "All Part site Combination should have all Std Alt UOMs. (KG/CV/PAC)",
...
#  Validator
# ══════════════════════════════════════════════
class PartUOMBusinessValidator:
    """Loads data, validates business rules, builds error map."""
    ...
    def load(self):
        print("[LOAD] Loading PartUOM file...")

        if PART_UOM_INPUT_FILE.lower().endswith((".xlsx", ".xls")):
            self.df = pd.read_excel(PART_UOM_INPUT_FILE, dtype=str)
        else:
            self.df = pd.read_csv(PART_UOM_INPUT_FILE, sep="\t", dtype=str)
    ...
    def validate(self):
        ...
        self.validate_alternativeunit_blank()
        self.validate_mandatory_uoms()

    def validate_alternativeunit_blank(self):
        rule = "ALTERNATIVEUNIT is blank"

        if "ALTERNATIVEUNIT" not in self.df.columns:
            return

        for idx, row in self.df.iterrows():
            if self._is_blank(row.get("ALTERNATIVEUNIT")):
                self._add_error(idx, "ALTERNATIVEUNIT", rule)

    def validate_mandatory_uoms(self):
        rule_base = "Std Alt UOMs missing. (KG/CV/PAC)"
        ...
        grouped = self.df.groupby(["PRODUCT", "PLANT"], sort=False, dropna=False)

        for (product, plant), group in grouped:
            product_clean = self._clean(product)
            plant_clean = self._clean(plant)

            if product_clean == "" or plant_clean == "":
                continue
            ...
            for _, row in group.iterrows():
                ...
                if not self._is_blank(alt_unit):
                    existing_uoms.add(str(alt_unit).strip().upper())

            missing_uoms = MANDATORY_UOMS - existing_uoms

            if missing_uoms:
                missing_text = ", ".join(sorted(missing_uoms))
                reason = f"{rule_base}. Missing UOMs: {missing_text}"

                # Record on first row of the PRODUCT + PLANT group
                first_idx = group.index[0]
                self._add_error(first_idx, "ALTERNATIVEUNIT", reason)
...
#  Report Writer
# ══════════════════════════════════════════════
class PartUOMBusinessReportWriter:
    """Builds business validation workbook."""

    SHEET_SUMMARY = "Summary"
    SHEET_RULESETS = "Rulesets"

    def __init__(self, validator: PartUOMBusinessValidator, output_path: str):
    ...
    def _write_ruleset_sheet(self, wb, summary_fields=None):
        ...
        title_cell = ws.cell(row=1, column=1, value="Part UOM Table – Business Validation Rules")
    ...
    def _write_summary_sheet(self, wb, error_map: dict, total_rows: int):
        ...
        title_cell = ws.cell(row=1, column=1, value="Part UOM Business Rules Summary")
        ...
        for bad_cols in error_map.values():
            for col, reason in bad_cols.items():
                ...
                reason_parts = str(reason).split(" | ")

                for single_reason in reason_parts:
                    col_error_counts[col] += 1
                    rule_error_counts[(col, single_reason)] = rule_error_counts.get((col, single_reason), 0) + 1
        ...
        for field_name, count in sorted_fields:
            reasons = BUSINESS_RULESET_INFO.get(field_name, [])
            ...
            if is_multi:
                ...
                ws.cell(row=row_num, column=2, value=field_name).font = BODY_FONT
                ...
                for reason in all_reasons:
                    ...
                    ws.cell(row=row_num, column=3, value=sub_count).font = BODY_FONT
                    ws.cell(row=row_num, column=4, value=total_rows).font = BODY_FONT
                    ...
                    ws.cell(row=row_num, column=7, value=reason if sub_count > 0 else "").font = BODY_FONT
                ...
                for c_idx, value in enumerate(values, start=1):
                    ...
                    cell.font = BODY_FONT
                    cell.border = THIN_BORDER
                    cell.alignment = Alignment(horizontal="center" if c_idx != 7 else "left", wrap_text=(c_idx == 7))
...
#  Orchestrator
# ══════════════════════════════════════════════
class PartUOMBusinessProcessor:
    def __init__(self):
        self.validator = PartUOMBusinessValidator()
        self.writer = PartUOMBusinessReportWriter(self.validator, OUTPUT_FILE)

    def run(self):
        print("=" * 70)
        print("Part UOM Business Validation Pipeline")
        print("=" * 70)
...
if __name__ == "__main__":
    processor = PartUOMBusinessProcessor()
    processor.run()
