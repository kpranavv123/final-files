...
# ─────────────────────────────────────────────
#  COLUMNS TO KEEP IN OUTPUT SHEETS
# ─────────────────────────────────────────────
KEEP_COLS = [
    "PURCHASEORDER",
    "DESTINATIONPLANT",
...
FIELD_REASON = {
    ...
    "POQUANTITY":                     "POQUANTITY: Field is blank",
    ...
    "TRANSITTIME":                    "",   # sub-rows carry reasons
}
...
class ScheduledReceiptRuleEngine:
    ...
    # ── 8. POQUANTITY ─────────────────────────
    def validate_poquantity(self, row) -> str:
        if self._is_blank(row.get("POQUANTITY")):
            return "POQUANTITY: Field is blank"
        return ""

    # ── 9. SCHEDULELINEDELIVERYDATE ───────────
    def validate_schedulelinedeliverydate(self, row) -> str:
        ...
        if not YYYYMMDD_PATTERN.match(val_str):
            return (
                f"SCHEDULELINEDELIVERYDATE: '{val_str}' does not follow the required "
                "format YYYYMMDD"
            )
        return ""

    # ── 10. NETPRICEINPURCHASINGDOCUMENTI ─────
    ...
    # ── 11. MATSTAGINGAVAILABILITYDATE ────────
    def validate_matstagingavailabilitydate(self, row) -> str:
        if self._is_blank(row.get("MATSTAGINGAVAILABILITYDATE")):
    ...
    def validate_transittime(self, row) -> str:
        ...
        if not YYYYMMDD_PATTERN.match(val_str):
            return (
                f"TRANSITTIME: '{val_str}' does not follow the required format YYYYMMDD"
            )
        return ""
    ...
    def get_rules(self) -> dict:
        return {
            ...
            "POQUANTITY":                    self.validate_poquantity,
...
class ScheduledReceiptValidator:
    ...
    def get_transittime_error_subcounts(self) -> dict:
        counts = {"blank": 0, "invalid_format": 0}
        for idx, col_reason in self.reason_map.items():
            reason = col_reason.get("TRANSITTIME", "")
            if not reason:
                continue
            if "blank" in reason.lower():
                counts["blank"] += 1
...
#  Report Writer
# ══════════════════════════════════════════════
class ScheduledReceiptReportWriter:
    ...
    def __init__(self, validator: ScheduledReceiptValidator, output_path: str):
    ...
    def _write_summary_sheet_into(self, ws, error_map: dict, total_rows: int):
        ...
        title_cell           = ws.cell(row=1, column=1,
                                       value="ScheduledReceipt (Sub-Con PO) Validation Summary")
        ...
        for col_name in FIELD_ORDER:
            ...
            if col_name == "DESTINATIONPLANT" and has_errors:
                sub_definitions = [
                    (
                        ...
                        dest_subcounts["blank"],
                        "DESTINATIONPLANT: Field is blank",
                    ),
                    (
                        "  ↳ Not in Site Master",
                        dest_subcounts["not_in_site"],
            ...
            # ── TRANSITTIME sub-rows ──
            if col_name == "TRANSITTIME" and has_errors:
                sub_definitions = [
                    (
                        "  ↳ Blank Transit Time",
                        tt_subcounts["blank"],
    ...
    def _write_rules_sheet(self, wb):
        ...
        title_cell           = ws.cell(row=1, column=1,
                                       value="ScheduledReceipt (Sub-Con PO) – Validation Rules")
