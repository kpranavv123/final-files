...
OUTPUT_FILE    = r"C:\Users\SW526XH\Downloads\Go Live-2\ScheduledReceipt(STO)\Validated_ScheduledReceipt_STO_Business.xlsx"


# ─────────────────────────────────────────────
#  STO EXTRACT COLUMN NAMES  (after .strip().upper())
# ─────────────────────────────────────────────
STO_QTY_COL = "POQUANTITYINBU"
...
#  Rule Engine
# ══════════════════════════════════════════════
class ScheduledReceiptSTOBusinessRuleEngine:
    """
    Rule:
      POQUANTITYINBU
    ...
    def validate_row(self, row) -> dict:
        """Returns {field_name: reason} for POQUANTITYINBU if the rule fails."""
        reasons = {}

        qty_raw = row.get(STO_QTY_COL, "")
...
#  Validator
# ══════════════════════════════════════════════
class ScheduledReceiptSTOBusinessTableValidator:

    def __init__(self, sto_path: str):
        self.sto_path            = sto_path
        self.df                  = pd.DataFrame()
        self.error_map           = {}   # row_idx -> [failed field names]
        self.reason_map          = {}   # row_idx -> {field: reason}

        # counter for Summary sheet
        self.negative_qty_count  = 0

    def load(self):
        self.df = pd.read_csv(self.sto_path, sep="\t", dtype=str)
        self.df.columns = [c.strip().upper() for c in self.df.columns]

    def validate(self):
        engine = ScheduledReceiptSTOBusinessRuleEngine()
...
#  Report Writer
# ══════════════════════════════════════════════
class ScheduledReceiptSTOBusinessReportWriter:
    ...
    def __init__(self, validator: ScheduledReceiptSTOBusinessTableValidator, output_path: str):
    ...
    # ══════════════════════════════════════════
    #  Summary sheet  (POQUANTITYINBU only — single rule, no sub-rows)
    # ══════════════════════════════════════════
    def _write_summary_sheet_into(self, ws, error_map: dict, total_rows: int):
        ...
        title_cell           = ws.cell(row=1, column=1,
                                        value="ScheduledReceipt (STO) Business Validation Summary")
    ...
    def _write_rules_sheet(self, wb):
        ...
        title_cell           = ws.cell(row=1, column=1,
                                        value="ScheduledReceipt (STO) – Business Validation Rules")
...
#  Orchestrator
# ══════════════════════════════════════════════
class ScheduledReceiptSTOBusinessTableProcessor:

    def __init__(self, sto_path: str, output_path: str):
        self.validator = ScheduledReceiptSTOBusinessTableValidator(sto_path)
        self.writer    = ScheduledReceiptSTOBusinessReportWriter(self.validator, output_path)
...
if __name__ == "__main__":
    processor = ScheduledReceiptSTOBusinessTableProcessor(
        sto_path    = STO_INPUT_FILE,
