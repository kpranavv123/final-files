...
# Part master — only MATERIALNUMBER needed (no plant combo for primary anymore)
...
# Customer master:
#   customer_plant_set  → "SUPPLYINGPLANT_CUSTOMER" strings  (used by HDA Primary PLANT_SOLDTOPARTY check)
#   customer_combo_set  → (CUSTOMER, SUPPLYINGPLANT) tuples  (used by HDA Secondary DISTRIBUTOR_CODE+PLANT check)
...
# Tuple set for Secondary combo check: (CUSTOMER, SUPPLYINGPLANT)
customer_combo_set = set(
    zip(
...
#  MATERIAL   : field must not be blank only (no Part master lookup)
#  PLANT      : must exist in Site master
#  PLANT_SOLDTOPARTY : combo must exist in Customer master
#  BILLING_DATE      : not blank, YYYYMMDD format
#  DUPLICATE_CHECK   : MATERIAL + PLANT + SOLDTOPARTY + BILLING_DATE
# ══════════════════════════════════════════════════════
PRIMARY_RULES = [
    ("MATERIAL",          "ERROR_MATERIAL",          "MATERIAL: Field must not be blank."),
    ("PLANT",             "ERROR_PLANT",             "PLANT: Plant is not present in Site master or is blank."),
    ("PLANT_SOLDTOPARTY", "ERROR_PLANT_SOLDTOPARTY", "PLANT_SOLDTOPARTY: Plant-Soldtoparty combination is not present in Customer master."),
    ("BILLING_DATE",      "ERROR_BILLING_DATE",      "BILLING_DATE: Must not be blank and must be in YYYYMMDD format."),
    ("DUPLICATE_CHECK",   "ERROR_DUPLICATE_CHECK",   "DUPLICATE_CHECK: Duplicate record — MATERIAL + PLANT + SOLDTOPARTY + BILLING_DATE combination already exists in the extract."),
]
...
PRIMARY_RULESET_DESC = {
    ...
    "PLANT_SOLDTOPARTY": [
        ...
        "Plant-Soldtoparty combination must exist in Customer master (SUPPLYINGPLANT + CUSTOMER).",
    ],
    "BILLING_DATE": [
        "Must not be blank.",
...
# ── Build composite key for PLANT_SOLDTOPARTY ────────────────────────────────
...
# ── Validation rules ─────────────────────────────────────────────────────────
# MATERIAL: field must not be blank only (no Part master lookup)
primary_df["ERROR_MATERIAL"] = primary_df["MATERIAL"].apply(
    lambda x: "Yes" if pd.isna(x) or str(x).strip() == "" else ""
)

primary_df["ERROR_PLANT"] = primary_df["PLANT"].apply(
    lambda x: "Yes" if pd.isna(x) or str(x).strip() == "" or x not in site_set else ""
)
...
primary_df["ERROR_BILLING_DATE"] = primary_df["BILLING_DATE"].apply(
    lambda x: "Yes" if pd.isna(x) or str(x).strip() == "" or not date_pattern.match(str(x)) else ""
)
...
#  DISTRIBUTOR_CODE : (DISTRIBUTOR_CODE + PLANT) combo must exist
#                     as (CUSTOMER + SUPPLYINGPLANT) in Customer master
#  PLANT            : must exist in Site master
#  INVOICE_DATE     : not blank, YYYYMMDD format
#  CSKU             : must exist in Part master
#  DUPLICATE_CHECK  : DISTRIBUTOR_CODE + PLANT + INVOICE_DATE + CSKU
# ══════════════════════════════════════════════════════
SECONDARY_RULES = [
    ("DISTRIBUTOR_CODE", "ERROR_DISTRIBUTOR_CODE", "DISTRIBUTOR_CODE: DISTRIBUTOR_CODE + PLANT combination does not exist in Customer master (CUSTOMER + SUPPLYINGPLANT) or field is blank."),
    ...
    ("CSKU",             "ERROR_CSKU",             "CSKU: CSKU missing in Part master or is blank."),
    ("DUPLICATE_CHECK",  "ERROR_DUPLICATE_CHECK",  "DUPLICATE_CHECK: Duplicate record — DISTRIBUTOR_CODE + PLANT + INVOICE_DATE + CSKU combination already exists in the extract."),
]
...
# ── Validation rules ─────────────────────────────────────────────────────────
# DISTRIBUTOR_CODE + PLANT combo must exist as (CUSTOMER, SUPPLYINGPLANT) in Customer master
secondary_df["ERROR_DISTRIBUTOR_CODE"] = secondary_df.apply(
    lambda row: "Yes"
    if (
        pd.isna(row["DISTRIBUTOR_CODE"]) or str(row["DISTRIBUTOR_CODE"]).strip() == ""
        or pd.isna(row["PLANT"])         or str(row["PLANT"]).strip() == ""
        or (str(row["DISTRIBUTOR_CODE"]).strip(), str(row["PLANT"]).strip()) not in customer_combo_set
    )
    else "",
    axis=1,
)
...
secondary_df["ERROR_INVOICE_DATE"] = secondary_df["INVOICE_DATE"].apply(
    lambda x: "Yes" if pd.isna(x) or str(x).strip() == "" or not date_pattern.fullmatch(str(x)) else ""
)

secondary_df["ERROR_CSKU"] = secondary_df["CSKU"].apply(
    lambda x: "Yes" if pd.isna(x) or str(x).strip() == "" or x not in part_set else ""
)
